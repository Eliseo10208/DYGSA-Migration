...
<div class="panel">
	...
	<form id="crear_config" class="form_s" data-ajax="false" method="post" enctype="multipart/form-data">
				...
				<div class="group">
					...
					<select name="camion" class="form-control">
				    	...
				    	if($camion->num_rows)
				    	{
				    		...
				    		{
				    			echo '<option value="'.$row['id'].'">'.$row['placa_rodaje'].'</option>';
				    		}
				    	}
						?>
					</select>
	...
	<script type="text/javascript">
		$('#crear_config').submit(function(e) {
			e.preventDefault();
			...
			$.ajax({
				type: 'post',
				url: `${site.url}/system/Actions/camion/conf.php`,
				data: values,
				success: function(m) {
					m = JSON.parse(m);
